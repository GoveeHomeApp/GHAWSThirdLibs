...
#修改spec文件tag
#获取版本号并显示
VersionString=`grep -E "s.version.*=" GHAWSThirdLibs.podspec`
...
LineNumber=`grep -nE 's.version.*=' GHAWSThirdLibs.podspec | cut -d : -f1`
sed -i "" "${LineNumber}s/${VersionNumber}/${NewVersionNumber}/g" GHAWSThirdLibs.podspec
echo -e "\n${Default}================================================"
